...
def image_to_base64_standard(image_path):
    """Encode image to base64 without smart_resize (for GPT-4o)."""
...
class GPT4Wrapper(LlmWrapper, MultimodalLlmWrapper):
    """Wrapper for GPT-4o via the standard OpenAI API (no smart_resize)."""
        ...
        for message in messages:
            ...
            for item in message['content']:
                if list(item.keys())[0] == 'text':
                    ...
                    new_content.append({
                        'type': 'image_url',
                        'image_url': {'url': image_to_base64_standard(item['image'])}
                    })
            converted_messages.append({'role': message['role'], 'content': new_content})
        return converted_messages
    ...
    def predict_mm(
        ...
        while counter > 0:
            try:
                resp = self.bot.chat.completions.create(model=self.model, messages=payload)
                return (resp.choices[0].message.content, payload, resp)
            except Exception as e:
                time.sleep(wait_seconds)
                wait_seconds *= 1
                counter -= 1
